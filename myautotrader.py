...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if (new_ask_price > self.max_price):
                self.max_price = new_ask_price
            if (new_bid_price < self.min_price):
                self.min_price = new_bid_price

            hi_to_lo_ratio = 100
            if (mid - self.min_price != 0):
                hi_to_lo_ratio = (self.max_price - mid) / \
                    (mid - self.min_price)
            lo_to_hi_ratio = 1 / hi_to_lo_ratio
            ...
            # mid closer to high
            # sell
            elif (hi_to_lo_ratio > 1.5):
                new_ask_price -= TICK_SIZE_IN_CENTS
                new_bid_price = mid
                # if (hi_to_lo_ratio > 4): sell_size = self.position * 0.8

            # mid closer to low
            # buy
            elif (lo_to_hi_ratio > 1.5):
                new_ask_price = mid
                new_bid_price += TICK_SIZE_IN_CENTS
                # if (lo_to_hi_ratio > 4): sell_size = self.position * 0.8

            if self.bid_id != 0:
                self.send_cancel_order(self.bid_id)
            self.bid_id = next(self.order_ids)
            self.send_insert_order(self.bid_id, Side.BUY, new_bid_price,
                                   buy_size, Lifespan.FILL_AND_KILL)
            ...
            self.send_insert_order(self.ask_id, Side.SELL, new_ask_price,
                                   sell_size, Lifespan.FILL_AND_KILL)
